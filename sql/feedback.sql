...
from (
  ...
  from
    ...
    left join (
      ...
      from
        geultto_6th_prod.message feedback
        join geultto_6th_prod.message submit on feedback.channel_id = submit.channel_id
  ...
  left join geultto_6th_prod.user on reviewee_id = user.user_id
group by
  due_ts, user_id
order by
  due_ts, user_id
