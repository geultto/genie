...
FROM (
  ...
  FROM (
    SELECT
      ...
      reviewee_id
    FROM
      geultto_5th_prod.review_mapping review,
      UNNEST(reviewee_ids) reviewee_id
    WHERE
      due_ts = '2020-11-29 15:00:00 UTC'
  ) r
  LEFT JOIN `geultto_5th_prod.user` u1 ON r.user_id = u1.user_id
  LEFT JOIN `geultto_5th_prod.user` u2 ON r.reviewee_id = u2.user_id
  LEFT JOIN (
      select
        user_id,
        concat("https://geultto5.slack.com/archives/", channel_id, "/p", unix_micros(ts)) as submit_m_url
      from
      ...
      where
        ...
        and due_ts = '2020-11-15 15:00:00 UTC'
